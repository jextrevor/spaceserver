    ...
    if (json.hasOwnProperty(key)) {
      ...
      if (key == "probing") {
        if (json[key] == "") {
          ...
          document.getElementById("probing").innerHTML =
            "Currently Probing: " +
            json[key] +
            " <button class='btn btn-warning' onclick=\"probe('')\">Cancel</button>";
        }
      }
      if (key == "probe") {
      ...
      if (key == "analysis") {
        if (json[key] == "") {
          ...
          document.getElementById("analysing").innerHTML =
            "Analysing Probing Data";
          document.getElementById("analysis").innerHTML = "";
        } else if (json[key] == "Sensors") {
          document.getElementById("analysing").innerHTML =
            "Analysing Sensors Data";
          document.getElementById("analysis").innerHTML = "";
        } else {
          document.getElementById("analysing").innerHTML = "";
      ...
      if (key == "scanning") {
        if (json[key] == "") {
          ...
          document.getElementById("scanning").innerHTML =
            "Currently Scanning: " +
            json[key] +
            " <button class='btn btn-warning' onclick=\"emit('update',{'scanning':''});sound('buttonshort.wav');\">Cancel</button>";
        }
      }
      if (key == "scan") {
...
function doships(json) {
  ...
  for (var key in json) {
    if (json.hasOwnProperty(key)) {
      document.getElementById("radarlist").innerHTML +=
        "<a class='list-group-item' onclick='probe(\"" +
        key +
        '");sound("processing.mp3");\'>' +
        key +
        "</a>";
    }
  }
...
function beam() {
  ...
  newjson[document.getElementById("transports").value].place =
...
function dolock() {
  if (
    data.objects[document.getElementById("outsides").value].lockable == true
  ) {
    lock = true;
